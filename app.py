...
def publish_product():
    ...
    retailerprice = request.form['retailerprice']
...
def handle_update(product_id):
    updated_data = request.form.to_dict()  # Get the updated data from the request
    if 'imgurl' in request.files:
        image = request.files['imgurl']
        if image.filename != '':
            image_id = fs.put(image, filename=image.filename)
            imgurl = str(image_id)
            updated_data['imgurl']= imgurl
    del updated_data['_id']
    result = db.product.update_one({'_id': ObjectId(product_id)}, {'$set': updated_data})
    if result.modified_count == 0:
        return {'message': 'Product not found'}, 404  # Return a 404 status code if product not found
    else:
        return {'message': 'Product updated successfully'}, 200  # Return a 200 status code if product updated successfully
...
def get_wish_elements():
    ...
    for wish_element in wish_elements:
        ...
        wish_element['category'] = product['category']
        wish_element['retailerprice'] = product['retailerprice']
...
def get_order_elements():
    user_id = request.args.get('userid')
    order_elements = list(db.orderelement.find({'userid':user_id}))
    for order_element in order_elements:
        product_id = order_element['productid']
        orderid = order_element['orderid']
        product = db.product.find_one({'_id': ObjectId(product_id)})
        recievedorder = db.recievedorder.find_one({'orderid':orderid})
        order_element['status'] = recievedorder['status']
        order_element['paymentdate'] = recievedorder['paymentdate']
...
def addcartelement():
    ...
    retailerprice = response1['retailerprice']
    res = db.store.find_one({"_id":ObjectId(storeid)})
    shippingprice = res['shippingprice']
    # Insert the data into the MongoDB collection
    post_data = {'retailerprice':retailerprice,"category":category,'userid': userid, 'productid': productid,'quantity':1,'shippingprice':shippingprice,"storeid":storeid}
...
def count_orders():
    ...
    if storeid:
        count = db.recievedorder.count_documents({'storeid': storeid})
        
        return jsonify({'count': count})
    else:
        return jsonify({'error': 'storeid parameter is missing'}), 400
...
def count_products():
    ...
    if storeid:
        # Query MongoDB to count products with the given storeid
        count = db.product.count_documents({'storeid': storeid})
...
@app.route('/store/visit', methods=['POST'])
def update_store_visitors():
    # Get storeid from request body
    storeid = request.json.get('storeid')

    # Check if the store exists in the database
    store = db.store.find_one({'_id': ObjectId(storeid)})
    if store:
        # Update the visitors count for the given storeid
        if 'visitors' in store:
            db.store.update_one(
        ...
        return jsonify({'message': f'Store with storeid {storeid} has been visited and visitors count updated.'})
    else:
        return jsonify({'message': f'Store with storeid {storeid} does not exist.'})

@app.route('/store/visit', methods=['GET'])
def get_store_visitors():
    # Get storeid from query parameters
    storeid = request.args.get('storeid')

    # Retrieve the visitors count for the given storeid
    store = db.store.find_one({'_id':ObjectId(storeid)})
    if store:
        visitors = store.get('visitors', 0)
        return jsonify({'visitors': visitors})
    else:
        return jsonify({'message': f'Store with storeid {storeid} not found.'}), 404
...
def getstoreusers():
    # Get storeid from request body
    storeid = request.json.get('storeid')
    unique_values = db.recievedorder.find({"storeid":storeid}).distinct('userid')
...
@app.route('/store_sales_revenue', methods=['post'])
def calculate_total_sales():
    storeid = request.json.get('storeid')
    total_sales = 0
    total_revenue =0 
    orders = db.orderelement.find({'storeid':storeid})

    for order in orders:
        quantity = int(order['quantity'])
        price  = int (order['price'])
        retailerprice =int(order['retailerprice'])
        pq = quantity*price
        prq= quantity*retailerprice
        total_sales += pq
        total_revenue += pq-prq
...
@app.route('/products/count/category/<storeid>', methods=['Get'])
def count_productscategory(storeid):
    # Get storeid from query parameters
    # Define the categories
    categories = ['Apparel and accessories', 'Electronics', 'Home and garden', 'Beauty and personal care',
                  'Sports and fitness', 'Toys and games', 'Food and beverages', 'Health and wellness']

    # Initialize a dictionary to store the counts
    counts = {category: 0 for category in categories}

    # Query the database to get products with the given storeid
    products = db.product.find({'storeid': storeid})

    # Iterate through the products and increment the counts based on their categories
    for product in products:
        category = product.get('category')  # Assuming the category field in the product document
        if category in categories:
            counts[category] += 1

    # Format the data as needed
    chartData = [{'label': category, 'value': count} for category, count in counts.items()]
...
@app.route('/sales_revenue_by_month/<storeid>', methods=['GET'])
def sales_revenue_by_month(storeid):
    year = 2023  # Get year from query parameters
    orders = db.orderelement.find({'storeid': storeid, 'date': {'$regex': '^{}-'.format(year)}})  # Filter orders by storeid and year
    sales_by_month = {}
    revenue_by_month = {}

    # Iterate through each order and calculate sales and revenue by month
    for order in orders:
        quantity = int(order['quantity'])
        price = int(order['price'])
        retailerprice = int(order['retailerprice'])
        ...
        month = order_date.strftime('%B') # Get month name without year

        if month in sales_by_month:
            sales_by_month[month] += pq
            revenue_by_month[month] += pq - prq
        else:
            sales_by_month[month] = pq
            revenue_by_month[month] = pq - prq

    # Create a list of dictionaries with month, total sales, and total revenue
    result = []
...
@app.route('/sales_by_month_year/<storeid>', methods=['GET'])
def sales_by_month_year(storeid):
    year1 = 2023
    year2 = 2022
    # Query the MongoDB collection for orders with matching storeid and date within the specified years
    start_date1 = datetime.datetime(int(year1), 1, 1).isoformat()
    end_date1 = datetime.datetime(int(year1), 12, 31).isoformat()
    start_date2 = datetime.datetime(int(year2), 1, 1).isoformat()
    end_date2 = datetime.datetime(int(year2), 12, 31).isoformat()

    orders1 = db.orderelement.find({'storeid': storeid, 'date': {'$gte': start_date1, '$lte': end_date1}})
    orders2 = db.orderelement.find({'storeid': storeid, 'date': {'$gte': start_date2, '$lte': end_date2}})

    # Initialize a dictionary to store sales by month for each year
    sales_by_month_year1 = {}
    sales_by_month_year2 = {}

    # Iterate through each order and calculate sales by month for each year
    for order in orders1:
        ...
        order_date = datetime.datetime.fromisoformat(order['date'])
        month = order_date.strftime('%B').lower()  # Format month as lowercase string

        if month in sales_by_month_year1:
            sales_by_month_year1[month] += pq
        else:
            sales_by_month_year1[month] = pq
    ...
    for order in orders2:
        ...
        if month in sales_by_month_year2:
            sales_by_month_year2[month] += pq
        else:
            sales_by_month_year2[month] = pq

    # Create a list of dictionaries with the desired format
    data1 = []
    for month in sales_by_month_year1:
        data1.append({
            'Month': month,
            f'Year({year1})': sales_by_month_year1[month],
            f'Year({year2})': sales_by_month_year2.get(month, 0)  # Use 0 if month not present in sales_by_month_year2
        })
